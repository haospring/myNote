...
定义在函数体内的内置类型的变量，如果不显示初始化，则它的值是未定义的，有一个不确定的值，但是编译并不会报错。
...
在函数体内部，初始化一个又 extern 关键字标记的变量，将报错
...
**对常量的引用**，常量引用一般用于参数列表，保证实参不会被形参改变
